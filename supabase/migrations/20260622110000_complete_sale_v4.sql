-- ============================================================================
-- Milestone 2 — complete_sale v4: SC/PWD discount + VAT exemption.
-- ============================================================================
...
declare
  ...
  v_total         integer;
  v_sc_pwd_disc   integer := 0;   -- computed SC/PWD discount
...
begin
  ...
  if v_dtype not in ('none', 'sc', 'pwd', 'manual') then
    v_dtype := 'none';
  end if;
  ...
  -- Gapless OR number (BIR CAS accreditation required before legally valid).
  ...
  loop
    ...
    -- SC/PWD: accumulate per-item discount (net-of-VAT × 20% × qty).
  ...
  -- SC/PWD sales are fully VAT-exempt; the entire net amount is vat_exempt.
  if v_dtype in ('sc', 'pwd') then
    v_vat_exempt := v_total;
  end if;
...
grant execute on function public.complete_sale(uuid, jsonb, integer, integer, public.payment_method, uuid, integer, text, text, text) to authenticated;
