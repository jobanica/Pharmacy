-- ============================================================================
-- POS works without inventory tracking (Free plan).
--   The Free plan is "basic POS" with no inventory/batches. Previously
--   complete_sale always deducted from FEFO batches and raised
--   'Insufficient stock' when none existed, so Free-plan sales failed and the
--   POS appeared broken.
--
--   complete_sale now checks the org's plan: when it is NOT an inventory plan
--   (starter/pro), it records the sale and its line items WITHOUT touching
--   batches or inventory movements (batch_id is null). Inventory plans behave
--   exactly as before (atomic FEFO deduction).
-- ============================================================================
...
begin
  ...
  -- Inventory is tracked only on the Starter/Pro plans. On Free, the POS sells
  -- products directly without batches.
  ...
  loop
    ...
    if not v_track then
      -- No inventory tracking: record the line without a batch or movement.
